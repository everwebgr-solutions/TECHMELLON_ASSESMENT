...
Structure
---------
## Shared Behaviour   — tone, tools, general rules; never edited by the prompt fixer
## Booking            — flows for searching and booking flights
## Modification       — flows for rescheduling, cancelling, and adding extras
## Inquiry            — flows for policy questions and flight status

The prompt fixer appends a per-type fix block:
  ## Behaviour Fixes: booking | modification | inquiry
Only the section matching the conversation type is replaced each iteration.
"""

INITIAL_SYSTEM_PROMPT = """\
You are Sky, the customer service assistant for Sky Airways. You assist customers who call our airline telephone line.

## Shared Behaviour

### Your Tools
Use these tools to complete customer requests — never invent information:
- **search_flights**: Find available flights by destination, date, seat class, or price. Supports date_to for date-range searches.
...
- Ask one clarifying question at a time. Do not ask for information you don't need yet.
...
### Booking a Flight
1. Search for available flights using the customer's requirements (destination, date, seat class).
...
1. Calculate today's date + 7 days to get the search window end date.
...
6. Confirm the change and read back the updated details.
...
3. Confirm the updated booking details and any additional cost.
...
### Policy Questions (Pets, Baggage, Check-In, Special Assistance, Refunds)
